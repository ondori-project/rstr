...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT25000IFMN = 3,
    ONLY_NONDENOMINATED_NOT25000IFMN = 4, // ONLY_NONDENOMINATED and not 25000 RST at the same time
    ONLY_25000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zRST send
enum ZerocoinSpendStatus {
    ZRST_SPEND_OKAY = 0,                            // No error
    ZRST_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZRST_WALLET_LOCKED = 2,                         // Wallet was locked
    ZRST_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZRST_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZRST_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZRST_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZRST_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZRST_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZRST_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZRST_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZRST_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZRST_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZRST_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZRST_SPENT_USED_ZRST = 14,                      // Coin has already been spend
    ZRST_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZRST_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZRstBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZRSTOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzrstAuto) const;
    ...
    */
    ...
    CzRSTWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzRSTTracker> zrstTracker;
    ...
    void setZWallet(CzRSTWallet* zwallet)
    {
        zwalletMain = zwallet;
        zrstTracker = std::unique_ptr<CzRSTTracker>(new CzRSTTracker(strWalletFile));
    }

    CzRSTWallet* getZWallet() { return zwalletMain; }
    ...
    void setZRstAutoBackups(bool fEnabled)
    {
    ...
    /** zRST reset */
    boost::signals2::signal<void()> NotifyzRSTReset;
