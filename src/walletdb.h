...
public:
    ...
    bool WriteZRSTSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZRSTSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZRSTSeed_deprecated(uint256& seed);
    bool EraseZRSTSeed();
    bool EraseZRSTSeed_deprecated();

    bool WriteZRSTCount(const uint32_t& nCount);
    bool ReadZRSTCount(uint32_t& nCount);
