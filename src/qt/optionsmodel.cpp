...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::RST);
    ...
    if (!settings.contains("nAnonymizeRSTRAmount"))
        settings.setValue("nAnonymizeRSTRAmount", 1000);

    nAnonymizeRSTRAmount = settings.value("nAnonymizeRSTRAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRSTRAmount"))
        SoftSetArg("-anonymizerstramount", settings.value("nAnonymizeRSTRAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rstr.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRSTRAmount:
            return QVariant(nAnonymizeRSTRAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRSTRAmount:
            nAnonymizeRSTRAmount = value.toInt();
            settings.setValue("nAnonymizeRSTRAmount", nAnonymizeRSTRAmount);
            emit anonymizeRSTRAmountChanged(nAnonymizeRSTRAmount);
            break;
        case CoinControlFeatures:
