...
#include "ui_zrstcontroldialog.h"
...
std::set<std::string> ZRstControlDialog::setSelectedMints;
std::set<CMintMeta> ZRstControlDialog::setMints;

ZRstControlDialog::ZRstControlDialog(QWidget *parent) :
	QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
	ui(new Ui::ZRstControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZRstControlDialog::~ZRstControlDialog()
{
    delete ui;
}

void ZRstControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRstControlDialog::updateList()
{
...
void ZRstControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRstControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRst_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRstControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRstControlDialog::GetSelectedMints()
{
...
void ZRstControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
