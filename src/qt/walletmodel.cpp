...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered rstr address / amount:
...

    {
        ...
        if (recipients[0].useQuickTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("QuickX doesn't support sending values that high yet. Transactions are currently limited to %1 RST.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal rstr:URI (rstr:XyZ...?message=example)
            {
...
static void NotifyzRSTReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzRSTReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzRSTReset.connect(boost::bind(NotifyzRSTReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzRSTReset.disconnect(boost::bind(NotifyzRSTReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zrstTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
