...
CzRSTWallet::CzRSTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrst seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRSTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRSTSeed_deprecated()) {
                    LogPrintf("%s: Updated zRST seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrst seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrst wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRSTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRSTCount(nCountLastUsed);
    else if (!walletdb.ReadZRSTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRSTWallet::Lock()
{
    seedMaster = 0;
}

void CzRSTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRSTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRST(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRSTWallet::LoadMintPoolFromDB()
{
...
void CzRSTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRSTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRSTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrstTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRSTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRST(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrstTracker which also adds to database
    pwalletMain->zrstTracker->Add(dMint, true);
...
void CzRSTWallet::SeedToZRST(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRSTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRSTWallet::GenerateDeterministicZRST(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRSTWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRSTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
