...
#ifndef RSTR_ZRSTWALLET_H
#define RSTR_ZRSTWALLET_H
...
public:
    CzRSTWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZRST(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZRST(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //RSTR_ZRSTWALLET_H
