...
// Copyright (c) 2016-2018 The RSTR developers
...
void ProcessMessageQuickTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "ix") {
        ...
        if (fAccepted) {
            ...
            if (GetTransactionLockSignatures(tx.GetHash()) == QUICKTX_SIGNATURES_REQUIRED) {
    ...
    } else if (strCommand == "txlvote") // QuickX Lock Consensus Votes
    {
        CConsensusVote ctx;
        vRecv >> ctx;
        ...
        if (mapTxLockReq.count(ctx.txHash) && GetTransactionLockSignatures(ctx.txHash) == QUICKTX_SIGNATURES_REQUIRED) {
...
bool IsIXTXValid(const CTransaction& txCollateral)
{
    ...
    if (nValueOut > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
        LogPrint("quickx", "IsIXTXValid - Transaction value too high - %s\n", txCollateral.ToString().c_str());
        return false;
    }

    if (missingTx) {
        LogPrint("quickx", "IsIXTXValid - Unknown inputs in IX transaction - %s\n", txCollateral.ToString().c_str());
        /*
    ...
    if (nValueIn - nValueOut < COIN * 0.01) {
        LogPrint("quickx", "IsIXTXValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString().c_str());
        return false;
    }
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())) {
        ...
        LogPrint("quickx", "CreateNewLock - Transaction Lock Exists %s !\n", tx.GetHash().ToString().c_str());
    }
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    if (n == -1) {
        LogPrint("quickx", "QuickX::DoConsensusVote - Unknown Masternode\n");
        return;
    }

    if (n > QUICKTX_SIGNATURES_TOTAL) {
        LogPrint("quickx", "QuickX::DoConsensusVote - Masternode not in the top %d (%d)\n", QUICKTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    LogPrint("quickx", "QuickX::DoConsensusVote - In the top %d (%d)\n", QUICKTX_SIGNATURES_TOTAL, n);
    ...
    if (!ctx.Sign()) {
        LogPrintf("QuickX::DoConsensusVote - Failed to sign consensus vote\n");
        return;
    }
    if (!ctx.SignatureValid()) {
        LogPrintf("QuickX::DoConsensusVote - Signature invalid\n");
        return;
    }
...
bool ProcessConsensusVote(CNode* pnode, CConsensusVote& ctx)
{
    ...
    if (pmn != NULL)
        LogPrint("quickx", "QuickX::ProcessConsensusVote - Masternode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("quickx", "QuickX::ProcessConsensusVote - Unknown Masternode\n");
        mnodeman.AskForMN(pnode, ctx.vinMasternode);
        return false;
    }

    if (n > QUICKTX_SIGNATURES_TOTAL) {
        LogPrint("quickx", "QuickX::ProcessConsensusVote - Masternode not in the top %d (%d) - %s\n", QUICKTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("QuickX::ProcessConsensusVote - Signature invalid\n");
    ...
    if (!mapTxLocks.count(ctx.txHash)) {
        LogPrintf("QuickX::ProcessConsensusVote - New Transaction Lock %s !\n", ctx.txHash.ToString().c_str());
        ...
        LogPrint("quickx", "QuickX::ProcessConsensusVote - Transaction Lock Exists %s !\n", ctx.txHash.ToString().c_str());
    ...
    if (i != mapTxLocks.end()) {
        ...
        LogPrint("quickx", "QuickX::ProcessConsensusVote - Transaction Lock Votes %d - %s !\n", (*i).second.CountSignatures(), ctx.GetHash().ToString().c_str());

        if ((*i).second.CountSignatures() >= QUICKTX_SIGNATURES_REQUIRED) {
            LogPrint("quickx", "QuickX::ProcessConsensusVote - Transaction Lock Is Complete %s !\n", (*i).second.GetHash().ToString().c_str());
    ...
    BOOST_FOREACH (const CTxIn& in, tx.vin) {
        if (mapLockedInputs.count(in.prevout)) {
            if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                LogPrintf("QuickX::CheckForConflictingLocks - found two complete conflicting locks - removing both. %s %s", tx.GetHash().ToString().c_str(), mapLockedInputs[in.prevout].ToString().c_str());
...
int GetTransactionLockSignatures(uint256 txHash)
{
    ...
    if (!IsSporkActive(SPORK_2_QUICKTX)) return -1;
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("QuickX::CConsensusVote::SignatureValid() - Unknown Masternode\n");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pmn->pubKeyMasternode, vchMasterNodeSignature, strMessage, errorMessage)) {
        LogPrintf("QuickX::CConsensusVote::SignatureValid() - Verify message failed\n");
        return false;
    }
