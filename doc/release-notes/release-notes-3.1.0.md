RSTR Core version *3.1.0* is now available from:  <https://github.com/rstr-project/rstr/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/rstr-project/rstr/issues>


Mandatory Update
==============

RSTR Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zRST Staking is here! zRST staking will be activated on the morning of the 8th of May 2018. With the release of zRST staking, there are effectively 2 versions of zRST, zRST minted on the 3.0.6 RSTR wallet or lower, and zRST minted on RSTR wallet version or higher. New features in this release will require the use of zRST v2, zRST minted on this wallet release 3.1.0 or later. If you currently hold zRST v1 and wish to take advantage of zRST staking and deterministic zRST, you will need to spend the zRST v1 to yourself and remint zRST v2.
Note: To find your zRST version, click the privacy tab, then the zRST Control button then expand the arrows next to the desired denomination.


### Deterministic zRST Seed Keys

zRST is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zRST outside of the wallet that the zRST had been minted on. zRST can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zRST minting

zRST minting now only requires 1 further mint (down from 2) to mature. zRST mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zRST Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zRST denomination has been spent or not.



RST/zRST Staking and Masternode Rewards
--------------

### RST, zRST and Masternode Payment Schedule

To encourage the use of zRST and increase the RSTR zerocoin anonymity set, the RSTR payment schedule has been changed to the following:

If a user staking zRST wins the reward for their block, the following zRST reward will be: 
- 3 zRST (3 x 1 denominations) rewarded to the staker, 2 RST rewarded to the masternode owner and 1 RST available for the budget. This is a total block reward of 6 RST, up from 5.

If a user staking RST wins the reward, the following amounts will be rewarded: 
- 2 RST to the RST staker, 3 RST to the Masternode owner and 1 RST available for the budget. This is a total block reward of 6 RST, up from 5.


### Return change to sender when minting zRST

Previously, zRST minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most RST and return the change from a zRST mint to that address.
...
The visual layout of the RSTR Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zRST minting feature. This is enabled by default and the enablezeromint=0 setting in the rstr.conf file will overwrite the GUI option.
-	The percentage of autominted zRST can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular RST.
...
### Summary
 ...
 The RSTR wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the rstr.conf file, found in the RSTR data directory.
- backuppath = <directory / full path>
- zrstbackuppath = <directory / full path>
...
* If zRST backup, auto generated name is `wallet-autozrstbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
RSTR Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The RSTR client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running rstrd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the RSTR support group, which often times sees users say "I didn't spend that zRST". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
Network Layer 2 Changes (Proposals / Budgets / QuickX)
--------------

### Monthly Budget Increase

As voted on by the RSTR masternodes, the monthly budget available to be utilised has been increased to 42,000 RST / month. This RST only has the opportunity to be raised once per month (paid to winning proposals) with any unused RST not created by the blockchain.

### Budget Finalization Fee

The RSTR finalization fee for successful proposals has now been reduced, this fee is now 5 RST down from 50 RST. The total fee outlay for a successful proposal is now a total of 55 RST.


### QuickX Raw Transactions

 When creating a raw transaction, it is now possible to create the transaction as a QuickX transaction. See the updated help documentation for the `createrawtransaction` RPC command.
...
-	Enforced v1 zRST spends to require a security level of 100
-	Updates to zRST spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the RSTR-Qt masternodes tab
-	Updated RSTR-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and RSTR's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zRST Control dialog in the QT wallet, a user is able to see zRST marked as version 1 or 2.

zPoS (zRST staking)
--------------

Once a zRST has over 200 confirmations it becomes available to stake. Staking zRST will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zRST. So for example if a 1,000 zRST denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zRSTs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within RSTR's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zRST, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zRST that can be recalculated at any time using the seed. Deterministic zRST allows for users to backup all of their future zRST by simply recording their seed and keeping it in a safe place (similar to backing up a private key for RST). The zRST seed needs to remain in the wallet in order to spend the zRST after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zRST data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

RSTR's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which RSTR used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to RSTR using the bug bounty program. The modulus is now correctly imported and RSTR's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zRST v2: zPoS dzRST ezRST external backup and budget fixes (too many to list)
...
### Build System
 ...
 - #461 `55ec19af0` [Build] libevent-rst dependency added to build-documentation (Mrs-X)
 
### P2P Protocol and Network Code
 - #542 `61156def7` [Network] Remove vfReachable and modify IsReachable to only use vfLimited. (Patrick Strateman)

### GUI
 - #572 `d9b23fe60` [Qt] Refresh zRST balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zRST status output (Mrs-X)
 - #499 `6305264f2` [Qt] Send popup simplified + QuickTX -> QuickX (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zRst Selected" labels after sending. (presstab)
 - #465 `d8e21774d` [Qt] Added controls to the options dialog for enable or disable auto-minting and set required level (lex-rst3)
 - #464 `59fd7d378` [Qt] setstakesplitthreshold value set in Qt GUI (lex-rst3)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault rstr-cli getinfo while loading block index (Mrs-X)
 ...
 - #423 `5dfb48ee6` [RPC] Add QuickX to rpcrawtransaction. (presstab)
 - #170 `027f16c64` [RPC] Convert source tree from json_spirit to UniValue (Fuzzbawls)

### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zRST spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zRST. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zRST. (presstab)
 
### Miscellaneous
 - #559 `d2b017217` [Bug] Segfault with -enablequicktx=0 / -enablequicktx=false (Mrs-X)
 ...
 - #463 `22f2bb623` [Tests] Fixed compilation errors (lex-rst3)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/rstr-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
