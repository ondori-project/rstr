...
*Setup instructions for a gitian build of RSTR using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the RSTR
...
Clone the git repositories for rstr and gitian and then checkout the rstr version that you want to build.
...
**Note**: if you've installed Gitian before May 16, 2015, please update to the latest version, see https://github.com/rstrandom/gitian-builder/issues/86
...
Follow the instructions in [doc/release-process.md](release-process.md) in the rstr repository
...
To build RSTR (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the rstr repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/rstr/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/rstr-crypto/rstr
...
For example:
```bash
URL=https://github.com/crowning-/rstr.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit RSTR=${COMMIT} --url rstr=${URL} ../RSTR/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit RSTR=${COMMIT} --url rstr=${URL} ../RSTR/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit RSTR=${COMMIT} --url rstr=${URL} ../RSTR/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/rstr-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/rstr-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/rstr-build.assert
```
...
[rstr/gitian.sigs](https://github.com/rstr-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
